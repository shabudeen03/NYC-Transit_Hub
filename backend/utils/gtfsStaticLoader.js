const axios = require("axios");
const fs = require("fs");
const path = require("path");
const unzipper = require("unzipper");
const csv = require("csv-parser");
const db = require('../database/db');

const GTFS_STATIC_URL = "https://rrgtfsfeeds.s3.amazonaws.com/gtfs_subway.zip";
const DATA_DIR = path.join(__dirname, "..", "data", "gtfs_data");

// Download the GTFS zip file
async function downloadGTFS() {
    const zipPath = path.join(DATA_DIR, "gtfs.zip");

    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR);

    const response = await axios({
        method: "GET",
        url: GTFS_STATIC_URL,
        responseType: "stream"
    });

    const writer = fs.createWriteStream(zipPath);

    response.data.pipe(writer);

    return new Promise((resolve, reject) => {
        writer.on("finish", () => resolve(zipPath));
        writer.on("error", reject);
    });
}

// Extract ZIP
async function extractGTFS(zipPath) {
    return fs
        .createReadStream(zipPath)
        .pipe(unzipper.Extract({ path: DATA_DIR }))
        .promise();
}

// Load stops.txt (stations)
async function loadFile(fileName) {
    const file = path.join(DATA_DIR, fileName);

    return new Promise((resolve, reject) => {
        const results = [];

        fs.createReadStream(file)
            .pipe(csv())
            .on("data", (data) => results.push(data))
            .on("end", () => resolve(results))
            .on("error", reject);
    });
}

async function getFile(name) {
    const data = await loadFile(name);
    return data;
}

async function getRoutesForStop(stopId) {
    const tripIds = db.prepare(`
        SELECT DISTINCT trip_id
        FROM stop_times
        WHERE stop_id = ?
    `).all(stopId).map(r => r.trip_id);

    if (tripIds.length === 0) return [];

    const placeholders = tripIds.map(() => '?').join(',');

    const routes = db.prepare(`
        SELECT DISTINCT route_id
        FROM trips
        WHERE trip_id IN (${placeholders})
    `).all(...tripIds).map(r => r.route_id);

    return routes;
}

function insertStopTimes(stopTimesData) {
    const insert = db.prepare(`
        INSERT OR IGNORE INTO stop_times (
            trip_id,
            arrival_time,
            departure_time,
            stop_id,
            stop_sequence
        ) VALUES (?, ?, ?, ?, ?)
    `);

    const insertMany = db.transaction((rows) => {
        for (const row of rows) {
            insert.run(
                row.trip_id,
                row.arrival_time,
                row.departure_time,
                row.stop_id,
                Number(row.stop_sequence)
            );
        }
    });

    insertMany(stopTimesData);
}

function insertTrips(tripsData) {
    const insert = db.prepare(`
        INSERT OR IGNORE INTO trips (
            route_id,
            service_id,
            trip_id,
            trip_headsign,
            direction_id,
            shape_id
        ) VALUES (?, ?, ?, ?, ?, ?)
    `);

    const insertMany = db.transaction((rows) => {
        for (const row of rows) {
            insert.run(
                row.route_id,
                row.service_id,
                row.trip_id,
                row.trip_headsign,
                row.direction_id,
                row.shape_id
            );
        }
    });

    insertMany(tripsData);
}

// missing: trains at a station
async function processStationData(stopsData) {
    // const trainStops = [];
    const query = db.prepare("INSERT INTO stations (stop_id, stop_name, stop_lat, stop_lon, location_type, parent_station, route_ids, accessibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?);");
    
    const insertMany = db.transaction((array) => {
        for(const stop of array) {
            if(stop.location_type === '' || stop.location_type === undefined) stop.location_type = null;
            if(stop.parent_station === '' || stop.parent_station === undefined) stop.parent_station = null;
            query.run(stop.stop_id, stop.stop_name, stop.stop_lat, stop.stop_lon, parseInt(stop.location_type), parseInt(stop.parent_station), null, null);
        }
    });

    // console.log(stopsData[0]);
    insertMany(stopsData);
    
    // for(const stop of stopsData) {
        // const routeIds = await getRoutesForStop(stop.stop_id);
        // stop.route_ids = routeIds;
    // }

    // console.log(trainStops[0]);
}

async function processRoutesData(routesData, tripsData) {
    const trainRoutes = new Map();
    const trsQuery = db.prepare("INSERT INTO train_shape_routes (route_id, shape_id) VALUES (?, ?);");
    const insertRouteShapes = db.transaction((rid, shapes) => {
        for(const shape of shapes) {
            // console.log(rid, shape);
            trsQuery.run(rid, shape[0]);
        }
    });

    for(const route of routesData) {
        const properRoutes = { ...route };

        const plannedShapes = new Map();
        for(const trip of tripsData) {
            if(trip.route_id === route.route_id) {
                // if(!plannedShapes.has(trip.shape_id) && trip.shape_id !== '' && trip.service_id === 'Weekday') {
                if(!plannedShapes.has(trip.shape_id)) {
                    // const value = { cnt: 1, sign: trip.trip_headsign }
                    const value = 1;
                    plannedShapes.set(trip.shape_id, value);
                } else {
                    let value = plannedShapes.get(trip.shape_id);
                    value++;
                    plannedShapes.set(trip.shape_id, value);
                }
            }
        }

        const sortedShapes = [...plannedShapes].sort((a, b) => b[1] - a[1]).filter(el => el[0] !== '');
        // const sortedShapes = [...plannedShapes].sort((a, b) => b[1] - a[1]);

        // if(sortedShapes.length === 0) console.log(route.route_id);
        // if(route.route_id === 'B') {
            // route.shape_id = 'b_train'
        // }
        // console.log(route.route_id, sortedShapes);

        // console.log(route.route_id, sortedShapes);
        // properRoutes.shape_ids = (sortedShapes.length > 0 ) ? JSON.stringify(sortedShapes) : "";
        // console.log(sortedShapes);
        
        if(properRoutes.route_id === 'B') {
            sortedShapes.push('b_train');
            // properRoutes.shape_ids = 'b_train'; //hardcode bus b train shape doesn't exist
        }

        insertRouteShapes(properRoutes.route_id, sortedShapes);
        trainRoutes.set(route.route_id, properRoutes);
    }

    // console.log(trainRoutes.get('1').shape_id);
    const query = db.prepare("INSERT INTO train_routes (route_id, agency_id, short_name, long_name, description, route_type, route_color, route_text_color) VALUES (?, ?, ?, ?, ?, ?, ?, ?);");

    const insertMany = db.transaction((hashmap) => {
        for(const routeId of hashmap.keys()) {
            const route = hashmap.get(routeId);

            // const jsonText = JSON.stringify(hashmap.get(shapeId));
            query.run(
                route.route_id,
                route.agency_id,
                route.short_name,
                route.long_name,
                route.description,
                parseInt(route.route_type),
                route.route_color,
                route.route_text_color,
                // route.shape_id
            );
        }
    });

    insertMany(trainRoutes);
}

async function processShapesData(shapesData) {
    const shapePoints = new Map();
    for(const shape of shapesData) {
        const coords = [ Number(shape.shape_pt_lat), Number(shape.shape_pt_lon) ];
        if(! shapePoints.has(shape.shape_id)) {
            shapePoints.set(shape.shape_id, [ coords ]);
        } else {
            const currList = shapePoints.get(shape.shape_id);
            currList.push(coords);
            shapePoints.set(shape.shape_id, currList);
        }
    }

    const query = db.prepare("INSERT INTO shapes (shape_id, coordinates) VALUES (?, ?);");

    const insertMany = db.transaction((hashmap) => {
        for(const shapeId of hashmap.keys()) {
            const jsonText = JSON.stringify(hashmap.get(shapeId));
            query.run(shapeId, jsonText);
        }
    });

    const bTrainCoords = [
    [
        40.873244,
        -73.887138
    ],
    [
        40.871801,
        -73.888279
    ],
    [
        40.871734,
        -73.888343
    ],
    [
        40.871668,
        -73.888408
    ],
    [
        40.871603,
        -73.888476
    ],
    [
        40.871539,
        -73.888546
    ],
    [
        40.871475,
        -73.888619
    ],
    [
        40.871413,
        -73.888695
    ],
    [
        40.871352,
        -73.888773
    ],
    [
        40.871292,
        -73.888854
    ],
    [
        40.871233,
        -73.888938
    ],
    [
        40.871175,
        -73.889024
    ],
    [
        40.871118,
        -73.889113
    ],
    [
        40.871061,
        -73.889205
    ],
    [
        40.871006,
        -73.889299
    ],
    [
        40.870952,
        -73.889396
    ],
    [
        40.870901,
        -73.889495
    ],
    [
        40.870411,
        -73.890297
    ],
    [
        40.870341,
        -73.890415
    ],
    [
        40.870266,
        -73.890532
    ],
    [
        40.870188,
        -73.890649
    ],
    [
        40.870106,
        -73.890765
    ],
    [
        40.87002,
        -73.89088
    ],
    [
        40.869931,
        -73.890995
    ],
    [
        40.869837,
        -73.891109
    ],
    [
        40.86974,
        -73.891223
    ],
    [
        40.869639,
        -73.891336
    ],
    [
        40.869533,
        -73.891448
    ],
    [
        40.869424,
        -73.89156
    ],
    [
        40.869312,
        -73.891671
    ],
    [
        40.869195,
        -73.891781
    ],
    [
        40.869074,
        -73.891891
    ],
    [
        40.868949,
        -73.891998
    ],
    [
        40.866978,
        -73.893509
    ],
    [
        40.866103,
        -73.894164
    ],
    [
        40.863108,
        -73.896631
    ],
    [
        40.863048,
        -73.896681
    ],
    [
        40.862986,
        -73.89673
    ],
    [
        40.862924,
        -73.896779
    ],
    [
        40.86286,
        -73.896827
    ],
    [
        40.862795,
        -73.896875
    ],
    [
        40.862728,
        -73.896923
    ],
    [
        40.86266,
        -73.89697
    ],
    [
        40.862591,
        -73.897017
    ],
    [
        40.862521,
        -73.897064
    ],
    [
        40.86245,
        -73.897111
    ],
    [
        40.862377,
        -73.897157
    ],
    [
        40.862303,
        -73.897202
    ],
    [
        40.862227,
        -73.897247
    ],
    [
        40.862151,
        -73.897292
    ],
    [
        40.862073,
        -73.897337
    ],
    [
        40.861296,
        -73.897749
    ],
    [
        40.859873,
        -73.898523
    ],
    [
        40.859807,
        -73.898557
    ],
    [
        40.859742,
        -73.898591
    ],
    [
        40.859676,
        -73.898625
    ],
    [
        40.85961,
        -73.89866
    ],
    [
        40.859544,
        -73.898695
    ],
    [
        40.859478,
        -73.898731
    ],
    [
        40.859412,
        -73.898768
    ],
    [
        40.859346,
        -73.898804
    ],
    [
        40.85928,
        -73.898842
    ],
    [
        40.859213,
        -73.89888
    ],
    [
        40.859147,
        -73.898918
    ],
    [
        40.859081,
        -73.898957
    ],
    [
        40.859014,
        -73.898997
    ],
    [
        40.858948,
        -73.899037
    ],
    [
        40.858881,
        -73.899078
    ],
    [
        40.856093,
        -73.900741
    ],
    [
        40.856016,
        -73.90079
    ],
    [
        40.855938,
        -73.90084
    ],
    [
        40.855862,
        -73.90089
    ],
    [
        40.855786,
        -73.90094
    ],
    [
        40.85571,
        -73.900992
    ],
    [
        40.855636,
        -73.901043
    ],
    [
        40.855562,
        -73.901096
    ],
    [
        40.855488,
        -73.901148
    ],
    [
        40.855416,
        -73.901202
    ],
    [
        40.855344,
        -73.901256
    ],
    [
        40.855273,
        -73.90131
    ],
    [
        40.855203,
        -73.901365
    ],
    [
        40.855133,
        -73.90142
    ],
    [
        40.855064,
        -73.901476
    ],
    [
        40.854996,
        -73.901532
    ],
    [
        40.85041,
        -73.905227
    ],
    [
        40.849058,
        -73.906311
    ],
    [
        40.848925,
        -73.906419
    ],
    [
        40.848791,
        -73.906534
    ],
    [
        40.848656,
        -73.906656
    ],
    [
        40.848518,
        -73.906787
    ],
    [
        40.848378,
        -73.906925
    ],
    [
        40.848236,
        -73.907072
    ],
    [
        40.848093,
        -73.907227
    ],
    [
        40.847947,
        -73.907389
    ],
    [
        40.847799,
        -73.90756
    ],
    [
        40.847649,
        -73.907739
    ],
    [
        40.847497,
        -73.907926
    ],
    [
        40.847343,
        -73.908121
    ],
    [
        40.847187,
        -73.908324
    ],
    [
        40.847028,
        -73.908535
    ],
    [
        40.846868,
        -73.908755
    ],
    [
        40.8459,
        -73.910136
    ],
    [
        40.845799,
        -73.910273
    ],
    [
        40.845695,
        -73.910404
    ],
    [
        40.845588,
        -73.910529
    ],
    [
        40.845478,
        -73.91065
    ],
    [
        40.845365,
        -73.910765
    ],
    [
        40.845249,
        -73.910875
    ],
    [
        40.845129,
        -73.91098
    ],
    [
        40.845006,
        -73.91108
    ],
    [
        40.84488,
        -73.911176
    ],
    [
        40.844751,
        -73.911266
    ],
    [
        40.844618,
        -73.911351
    ],
    [
        40.844482,
        -73.91143
    ],
    [
        40.844343,
        -73.911505
    ],
    [
        40.844201,
        -73.911575
    ],
    [
        40.844055,
        -73.91164
    ],
    [
        40.840469,
        -73.91285
    ],
    [
        40.840377,
        -73.912888
    ],
    [
        40.840288,
        -73.912924
    ],
    [
        40.8402,
        -73.912961
    ],
    [
        40.840115,
        -73.912997
    ],
    [
        40.840031,
        -73.913034
    ],
    [
        40.839949,
        -73.91307
    ],
    [
        40.83987,
        -73.913107
    ],
    [
        40.839793,
        -73.913143
    ],
    [
        40.839717,
        -73.91318
    ],
    [
        40.839644,
        -73.913216
    ],
    [
        40.839572,
        -73.913253
    ],
    [
        40.839503,
        -73.913289
    ],
    [
        40.839436,
        -73.913326
    ],
    [
        40.83937,
        -73.913362
    ],
    [
        40.839306,
        -73.9134
    ],
    [
        40.839202,
        -73.91346
    ],
    [
        40.839097,
        -73.913522
    ],
    [
        40.838992,
        -73.913588
    ],
    [
        40.838887,
        -73.913656
    ],
    [
        40.838781,
        -73.913728
    ],
    [
        40.838674,
        -73.913802
    ],
    [
        40.838567,
        -73.913879
    ],
    [
        40.838459,
        -73.91396
    ],
    [
        40.838351,
        -73.914043
    ],
    [
        40.838242,
        -73.914129
    ],
    [
        40.838133,
        -73.914218
    ],
    [
        40.838023,
        -73.91431
    ],
    [
        40.837912,
        -73.914405
    ],
    [
        40.837801,
        -73.914503
    ],
    [
        40.837691,
        -73.914603
    ],
    [
        40.833771,
        -73.91844
    ],
    [
        40.833604,
        -73.918591
    ],
    [
        40.833436,
        -73.918744
    ],
    [
        40.833263,
        -73.918895
    ],
    [
        40.833085,
        -73.919043
    ],
    [
        40.832902,
        -73.91919
    ],
    [
        40.832715,
        -73.919334
    ],
    [
        40.832523,
        -73.919475
    ],
    [
        40.832326,
        -73.919614
    ],
    [
        40.832124,
        -73.919751
    ],
    [
        40.831918,
        -73.919886
    ],
    [
        40.831707,
        -73.920018
    ],
    [
        40.831491,
        -73.920148
    ],
    [
        40.83127,
        -73.920275
    ],
    [
        40.831044,
        -73.9204
    ],
    [
        40.830814,
        -73.920523
    ],
    [
        40.828524,
        -73.921612
    ],
    [
        40.828304,
        -73.921711
    ],
    [
        40.828106,
        -73.921823
    ],
    [
        40.827931,
        -73.921951
    ],
    [
        40.827779,
        -73.922092
    ],
    [
        40.82765,
        -73.922248
    ],
    [
        40.827543,
        -73.922419
    ],
    [
        40.827459,
        -73.922603
    ],
    [
        40.827397,
        -73.922802
    ],
    [
        40.827358,
        -73.923016
    ],
    [
        40.827342,
        -73.923244
    ],
    [
        40.827348,
        -73.923486
    ],
    [
        40.827378,
        -73.923742
    ],
    [
        40.827429,
        -73.924013
    ],
    [
        40.827504,
        -73.924298
    ],
    [
        40.827601,
        -73.924598
    ],
    [
        40.827905,
        -73.925651
    ],
    [
        40.827944,
        -73.925782
    ],
    [
        40.827984,
        -73.925912
    ],
    [
        40.828025,
        -73.92604
    ],
    [
        40.828067,
        -73.926166
    ],
    [
        40.82811,
        -73.926292
    ],
    [
        40.828154,
        -73.926417
    ],
    [
        40.828199,
        -73.92654
    ],
    [
        40.828245,
        -73.926662
    ],
    [
        40.828292,
        -73.926783
    ],
    [
        40.828341,
        -73.926902
    ],
    [
        40.82839,
        -73.927021
    ],
    [
        40.82844,
        -73.927138
    ],
    [
        40.828491,
        -73.927254
    ],
    [
        40.828544,
        -73.927369
    ],
    [
        40.828599,
        -73.927482
    ],
    [
        40.829514,
        -73.929053
    ],
    [
        40.829569,
        -73.929156
    ],
    [
        40.829618,
        -73.929263
    ],
    [
        40.829664,
        -73.929375
    ],
    [
        40.829708,
        -73.929492
    ],
    [
        40.829748,
        -73.929613
    ],
    [
        40.829785,
        -73.929738
    ],
    [
        40.829819,
        -73.929868
    ],
    [
        40.82985,
        -73.930003
    ],
    [
        40.829878,
        -73.930142
    ],
    [
        40.829904,
        -73.930285
    ],
    [
        40.829926,
        -73.930434
    ],
    [
        40.829945,
        -73.930586
    ],
    [
        40.829961,
        -73.930743
    ],
    [
        40.829975,
        -73.930905
    ],
    [
        40.829985,
        -73.931071
    ],
    [
        40.830093,
        -73.936658
    ],
    [
        40.830135,
        -73.938209
    ],
    [
        40.830135,
        -73.938273
    ],
    [
        40.830134,
        -73.938335
    ],
    [
        40.830134,
        -73.938398
    ],
    [
        40.830133,
        -73.938461
    ],
    [
        40.830132,
        -73.938524
    ],
    [
        40.83013,
        -73.938587
    ],
    [
        40.830129,
        -73.93865
    ],
    [
        40.830127,
        -73.938713
    ],
    [
        40.830125,
        -73.938776
    ],
    [
        40.830123,
        -73.938839
    ],
    [
        40.83012,
        -73.938902
    ],
    [
        40.830118,
        -73.938965
    ],
    [
        40.830115,
        -73.939028
    ],
    [
        40.830111,
        -73.939092
    ],
    [
        40.830108,
        -73.939156
    ],
    [
        40.830091,
        -73.939326
    ],
    [
        40.830069,
        -73.939489
    ],
    [
        40.830041,
        -73.939646
    ],
    [
        40.830007,
        -73.939797
    ],
    [
        40.829967,
        -73.939941
    ],
    [
        40.829922,
        -73.940079
    ],
    [
        40.829871,
        -73.940211
    ],
    [
        40.829815,
        -73.940336
    ],
    [
        40.829753,
        -73.940455
    ],
    [
        40.829685,
        -73.940567
    ],
    [
        40.829612,
        -73.940673
    ],
    [
        40.829533,
        -73.940772
    ],
    [
        40.829448,
        -73.940866
    ],
    [
        40.829358,
        -73.940953
    ],
    [
        40.829263,
        -73.941032
    ],
    [
        40.827318,
        -73.942289
    ],
    [
        40.827301,
        -73.9423
    ],
    [
        40.827283,
        -73.942311
    ],
    [
        40.827266,
        -73.942322
    ],
    [
        40.827248,
        -73.942334
    ],
    [
        40.827231,
        -73.942346
    ],
    [
        40.827213,
        -73.942358
    ],
    [
        40.827196,
        -73.942371
    ],
    [
        40.827178,
        -73.942384
    ],
    [
        40.827161,
        -73.942397
    ],
    [
        40.827143,
        -73.94241
    ],
    [
        40.827126,
        -73.942424
    ],
    [
        40.827108,
        -73.942438
    ],
    [
        40.82709,
        -73.942453
    ],
    [
        40.827073,
        -73.942468
    ],
    [
        40.827054,
        -73.942486
    ],
    [
        40.825903,
        -73.943403
    ],
    [
        40.824882,
        -73.944148
    ],
    [
        40.824783,
        -73.944216
    ],
    [
        40.824264,
        -73.944591
    ],
    [
        40.824231,
        -73.944617
    ],
    [
        40.824197,
        -73.944643
    ],
    [
        40.824163,
        -73.944669
    ],
    [
        40.824127,
        -73.944694
    ],
    [
        40.824092,
        -73.944719
    ],
    [
        40.824055,
        -73.944743
    ],
    [
        40.824018,
        -73.944767
    ],
    [
        40.82398,
        -73.944791
    ],
    [
        40.823941,
        -73.944814
    ],
    [
        40.823902,
        -73.944837
    ],
    [
        40.823862,
        -73.94486
    ],
    [
        40.823822,
        -73.944882
    ],
    [
        40.823781,
        -73.944904
    ],
    [
        40.823739,
        -73.944926
    ],
    [
        40.823696,
        -73.944947
    ],
    [
        40.817894,
        -73.947649
    ],
    [
        40.815297,
        -73.948857
    ],
    [
        40.815254,
        -73.948875
    ],
    [
        40.815212,
        -73.948895
    ],
    [
        40.815171,
        -73.948914
    ],
    [
        40.81513,
        -73.948935
    ],
    [
        40.815089,
        -73.948956
    ],
    [
        40.815049,
        -73.948977
    ],
    [
        40.815009,
        -73.949
    ],
    [
        40.814969,
        -73.949023
    ],
    [
        40.81493,
        -73.949046
    ],
    [
        40.814892,
        -73.94907
    ],
    [
        40.814853,
        -73.949095
    ],
    [
        40.814815,
        -73.949121
    ],
    [
        40.814778,
        -73.949147
    ],
    [
        40.814741,
        -73.949174
    ],
    [
        40.814704,
        -73.949202
    ],
    [
        40.813459,
        -73.950112
    ],
    [
        40.813416,
        -73.950141
    ],
    [
        40.813376,
        -73.950172
    ],
    [
        40.813335,
        -73.950203
    ],
    [
        40.813296,
        -73.950234
    ],
    [
        40.813257,
        -73.950266
    ],
    [
        40.813219,
        -73.950298
    ],
    [
        40.813181,
        -73.950331
    ],
    [
        40.813145,
        -73.950363
    ],
    [
        40.813109,
        -73.950397
    ],
    [
        40.813073,
        -73.95043
    ],
    [
        40.813038,
        -73.950464
    ],
    [
        40.813004,
        -73.950498
    ],
    [
        40.812971,
        -73.950533
    ],
    [
        40.812938,
        -73.950568
    ],
    [
        40.812906,
        -73.950604
    ],
    [
        40.811923,
        -73.951586
    ],
    [
        40.811871,
        -73.951645
    ],
    [
        40.811819,
        -73.951701
    ],
    [
        40.811767,
        -73.951757
    ],
    [
        40.811714,
        -73.951811
    ],
    [
        40.811661,
        -73.951865
    ],
    [
        40.811608,
        -73.951917
    ],
    [
        40.811554,
        -73.951969
    ],
    [
        40.8115,
        -73.952019
    ],
    [
        40.811445,
        -73.952068
    ],
    [
        40.81139,
        -73.952117
    ],
    [
        40.811335,
        -73.952164
    ],
    [
        40.811279,
        -73.95221
    ],
    [
        40.811223,
        -73.952256
    ],
    [
        40.811167,
        -73.9523
    ],
    [
        40.811109,
        -73.952343
    ],
    [
        40.810422,
        -73.952842
    ],
    [
        40.810377,
        -73.952872
    ],
    [
        40.810332,
        -73.952898
    ],
    [
        40.810285,
        -73.952922
    ],
    [
        40.810237,
        -73.952943
    ],
    [
        40.810188,
        -73.952963
    ],
    [
        40.810138,
        -73.95298
    ],
    [
        40.810087,
        -73.952995
    ],
    [
        40.810035,
        -73.953007
    ],
    [
        40.809981,
        -73.953017
    ],
    [
        40.809927,
        -73.953025
    ],
    [
        40.809872,
        -73.95303
    ],
    [
        40.809815,
        -73.953033
    ],
    [
        40.809757,
        -73.953034
    ],
    [
        40.809699,
        -73.953033
    ],
    [
        40.809638,
        -73.95303
    ],
    [
        40.808245,
        -73.952941
    ],
    [
        40.808177,
        -73.952939
    ],
    [
        40.808109,
        -73.952939
    ],
    [
        40.808042,
        -73.952942
    ],
    [
        40.807975,
        -73.95295
    ],
    [
        40.807908,
        -73.952961
    ],
    [
        40.807842,
        -73.952976
    ],
    [
        40.807777,
        -73.952995
    ],
    [
        40.807711,
        -73.953018
    ],
    [
        40.807647,
        -73.953045
    ],
    [
        40.807583,
        -73.953075
    ],
    [
        40.807519,
        -73.95311
    ],
    [
        40.807455,
        -73.953148
    ],
    [
        40.807392,
        -73.95319
    ],
    [
        40.80733,
        -73.953236
    ],
    [
        40.807267,
        -73.953288
    ],
    [
        40.805085,
        -73.954882
    ],
    [
        40.800603,
        -73.958161
    ],
    [
        40.796092,
        -73.961454
    ],
    [
        40.791642,
        -73.964696
    ],
    [
        40.785868,
        -73.968916
    ],
    [
        40.781433,
        -73.972143
    ],
    [
        40.775594,
        -73.97641
    ],
    [
        40.768296,
        -73.981736
    ],
    [
        40.764986,
        -73.984138
    ],
    [
        40.764883,
        -73.984192
    ],
    [
        40.764782,
        -73.984234
    ],
    [
        40.764684,
        -73.984265
    ],
    [
        40.764589,
        -73.984283
    ],
    [
        40.764497,
        -73.984289
    ],
    [
        40.764408,
        -73.984284
    ],
    [
        40.764321,
        -73.984266
    ],
    [
        40.764237,
        -73.984237
    ],
    [
        40.764157,
        -73.984195
    ],
    [
        40.764078,
        -73.984142
    ],
    [
        40.764003,
        -73.984077
    ],
    [
        40.763931,
        -73.983999
    ],
    [
        40.763861,
        -73.98391
    ],
    [
        40.763794,
        -73.983809
    ],
    [
        40.763731,
        -73.983698
    ],
    [
        40.762874,
        -73.981663
    ],
    [
        40.762862,
        -73.981637
    ],
    [
        40.762259,
        -73.980212
    ],
    [
        40.762191,
        -73.980073
    ],
    [
        40.762121,
        -73.97995
    ],
    [
        40.762047,
        -73.979842
    ],
    [
        40.761969,
        -73.979749
    ],
    [
        40.761889,
        -73.97967
    ],
    [
        40.761805,
        -73.979607
    ],
    [
        40.761717,
        -73.979558
    ],
    [
        40.761626,
        -73.979524
    ],
    [
        40.761532,
        -73.979505
    ],
    [
        40.761434,
        -73.9795
    ],
    [
        40.761332,
        -73.97951
    ],
    [
        40.761228,
        -73.979536
    ],
    [
        40.76112,
        -73.979576
    ],
    [
        40.761008,
        -73.97963
    ],
    [
        40.760893,
        -73.979701
    ],
    [
        40.758663,
        -73.981329
    ],
    [
        40.754222,
        -73.984569
    ],
    [
        40.749719,
        -73.987823
    ],
    [
        40.742878,
        -73.992821
    ],
    [
        40.738228,
        -73.996209
    ],
    [
        40.733324,
        -73.999777
    ],
    [
        40.732362,
        -74.000477
    ],
    [
        40.732338,
        -74.000495
    ],
    [
        40.732312,
        -74.000515
    ],
    [
        40.731216,
        -74.001312
    ],
    [
        40.731189,
        -74.001331
    ],
    [
        40.731161,
        -74.001349
    ],
    [
        40.731134,
        -74.001367
    ],
    [
        40.731107,
        -74.001385
    ],
    [
        40.73108,
        -74.001402
    ],
    [
        40.731054,
        -74.001419
    ],
    [
        40.731027,
        -74.001435
    ],
    [
        40.731001,
        -74.001451
    ],
    [
        40.730975,
        -74.001467
    ],
    [
        40.730948,
        -74.001482
    ],
    [
        40.730922,
        -74.001497
    ],
    [
        40.730897,
        -74.001511
    ],
    [
        40.730871,
        -74.001525
    ],
    [
        40.730845,
        -74.001539
    ],
    [
        40.73082,
        -74.001552
    ],
    [
        40.729279,
        -74.002395
    ],
    [
        40.729154,
        -74.002446
    ],
    [
        40.729034,
        -74.002486
    ],
    [
        40.728918,
        -74.002515
    ],
    [
        40.728808,
        -74.002531
    ],
    [
        40.728702,
        -74.002535
    ],
    [
        40.728601,
        -74.002526
    ],
    [
        40.728505,
        -74.002506
    ],
    [
        40.728414,
        -74.002473
    ],
    [
        40.728328,
        -74.002428
    ],
    [
        40.728247,
        -74.002371
    ],
    [
        40.72817,
        -74.002301
    ],
    [
        40.728099,
        -74.00222
    ],
    [
        40.728032,
        -74.002126
    ],
    [
        40.72797,
        -74.002019
    ],
    [
        40.727913,
        -74.001901
    ],
    [
        40.726047,
        -73.998114
    ],
    [
        40.726004,
        -73.998027
    ],
    [
        40.725961,
        -73.997938
    ],
    [
        40.725918,
        -73.997848
    ],
    [
        40.725876,
        -73.997756
    ],
    [
        40.725834,
        -73.997663
    ],
    [
        40.725792,
        -73.997568
    ],
    [
        40.725751,
        -73.997471
    ],
    [
        40.725711,
        -73.997373
    ],
    [
        40.72567,
        -73.997273
    ],
    [
        40.725631,
        -73.997172
    ],
    [
        40.725591,
        -73.997069
    ],
    [
        40.725552,
        -73.996964
    ],
    [
        40.725513,
        -73.996858
    ],
    [
        40.725475,
        -73.99675
    ],
    [
        40.725437,
        -73.996641
    ],
    [
        40.725297,
        -73.996204
    ],
    [
        40.724185,
        -73.99255
    ],
    [
        40.724133,
        -73.992422
    ],
    [
        40.724075,
        -73.992304
    ],
    [
        40.724013,
        -73.992196
    ],
    [
        40.723946,
        -73.992098
    ],
    [
        40.723875,
        -73.992011
    ],
    [
        40.7238,
        -73.991935
    ],
    [
        40.72372,
        -73.991868
    ],
    [
        40.723635,
        -73.991812
    ],
    [
        40.723546,
        -73.991767
    ],
    [
        40.723453,
        -73.991731
    ],
    [
        40.723355,
        -73.991706
    ],
    [
        40.723253,
        -73.991691
    ],
    [
        40.723147,
        -73.991687
    ],
    [
        40.723036,
        -73.991693
    ],
    [
        40.722918,
        -73.991709
    ],
    [
        40.72208,
        -73.991861
    ],
    [
        40.722052,
        -73.991868
    ],
    [
        40.722024,
        -73.991875
    ],
    [
        40.721997,
        -73.991883
    ],
    [
        40.721969,
        -73.991891
    ],
    [
        40.721941,
        -73.9919
    ],
    [
        40.721912,
        -73.991909
    ],
    [
        40.721884,
        -73.99192
    ],
    [
        40.721856,
        -73.991931
    ],
    [
        40.721827,
        -73.991942
    ],
    [
        40.721799,
        -73.991954
    ],
    [
        40.72177,
        -73.991967
    ],
    [
        40.721742,
        -73.991981
    ],
    [
        40.721713,
        -73.991995
    ],
    [
        40.721684,
        -73.99201
    ],
    [
        40.721655,
        -73.992025
    ],
    [
        40.718267,
        -73.993753
    ],
    [
        40.716401,
        -73.994714
    ],
    [
        40.716296,
        -73.994767
    ],
    [
        40.716191,
        -73.994812
    ],
    [
        40.716087,
        -73.994849
    ],
    [
        40.715984,
        -73.994879
    ],
    [
        40.715881,
        -73.994901
    ],
    [
        40.715779,
        -73.994916
    ],
    [
        40.715678,
        -73.994923
    ],
    [
        40.715578,
        -73.994923
    ],
    [
        40.715478,
        -73.994915
    ],
    [
        40.715379,
        -73.9949
    ],
    [
        40.715281,
        -73.994877
    ],
    [
        40.715184,
        -73.994847
    ],
    [
        40.715087,
        -73.99481
    ],
    [
        40.714991,
        -73.994765
    ],
    [
        40.714897,
        -73.994713
    ],
    [
        40.697839,
        -73.985242
    ],
    [
        40.695138,
        -73.983934
    ],
    [
        40.693829,
        -73.983428
    ],
    [
        40.691559,
        -73.982304
    ],
    [
        40.690635,
        -73.981824
    ],
    [
        40.690635,
        -73.981824
    ],
    [
        40.689405,
        -73.981205
    ],
    [
        40.689345,
        -73.981179
    ],
    [
        40.689287,
        -73.981149
    ],
    [
        40.689232,
        -73.981117
    ],
    [
        40.689178,
        -73.981081
    ],
    [
        40.689127,
        -73.981042
    ],
    [
        40.689078,
        -73.980999
    ],
    [
        40.689031,
        -73.980954
    ],
    [
        40.688986,
        -73.980905
    ],
    [
        40.688943,
        -73.980853
    ],
    [
        40.688902,
        -73.980798
    ],
    [
        40.688863,
        -73.98074
    ],
    [
        40.688827,
        -73.980678
    ],
    [
        40.688792,
        -73.980613
    ],
    [
        40.68876,
        -73.980545
    ],
    [
        40.68873,
        -73.980474
    ],
    [
        40.688068,
        -73.978805
    ],
    [
        40.687703,
        -73.977928
    ],
    [
        40.68768,
        -73.977868
    ],
    [
        40.687656,
        -73.977812
    ],
    [
        40.68763,
        -73.97776
    ],
    [
        40.687602,
        -73.977711
    ],
    [
        40.687573,
        -73.977666
    ],
    [
        40.687541,
        -73.977624
    ],
    [
        40.687508,
        -73.977587
    ],
    [
        40.687473,
        -73.977552
    ],
    [
        40.687437,
        -73.977522
    ],
    [
        40.687399,
        -73.977495
    ],
    [
        40.687359,
        -73.977472
    ],
    [
        40.687317,
        -73.977452
    ],
    [
        40.687273,
        -73.977437
    ],
    [
        40.687228,
        -73.977424
    ],
    [
        40.687181,
        -73.977416
    ],
    [
        40.68446,
        -73.97689
    ],
    [
        40.683433,
        -73.976708
    ],
    [
        40.683376,
        -73.976698
    ],
    [
        40.68332,
        -73.976687
    ],
    [
        40.683264,
        -73.976675
    ],
    [
        40.683208,
        -73.976661
    ],
    [
        40.683152,
        -73.976645
    ],
    [
        40.683097,
        -73.976628
    ],
    [
        40.683041,
        -73.976609
    ],
    [
        40.682987,
        -73.976589
    ],
    [
        40.682932,
        -73.976566
    ],
    [
        40.682877,
        -73.976543
    ],
    [
        40.682823,
        -73.976517
    ],
    [
        40.682769,
        -73.97649
    ],
    [
        40.682716,
        -73.976461
    ],
    [
        40.682662,
        -73.976431
    ],
    [
        40.682608,
        -73.976398
    ],
    [
        40.67705,
        -73.972367
    ],
    [
        40.663876,
        -73.962927
    ],
    [
        40.663791,
        -73.962872
    ],
    [
        40.663705,
        -73.96282
    ],
    [
        40.663619,
        -73.962771
    ],
    [
        40.663532,
        -73.962724
    ],
    [
        40.663445,
        -73.962681
    ],
    [
        40.663357,
        -73.96264
    ],
    [
        40.663269,
        -73.962601
    ],
    [
        40.663181,
        -73.962566
    ],
    [
        40.663092,
        -73.962533
    ],
    [
        40.663003,
        -73.962502
    ],
    [
        40.662913,
        -73.962475
    ],
    [
        40.662823,
        -73.96245
    ],
    [
        40.662733,
        -73.962427
    ],
    [
        40.662642,
        -73.962408
    ],
    [
        40.662551,
        -73.962392
    ],
    [
        40.661614,
        -73.962246
    ],
    [
        40.66101,
        -73.96215
    ],
    [
        40.660997,
        -73.962148
    ],
    [
        40.660985,
        -73.962146
    ],
    [
        40.660972,
        -73.962144
    ],
    [
        40.660959,
        -73.962141
    ],
    [
        40.660947,
        -73.962139
    ],
    [
        40.660935,
        -73.962136
    ],
    [
        40.660922,
        -73.962133
    ],
    [
        40.66091,
        -73.962131
    ],
    [
        40.660898,
        -73.962128
    ],
    [
        40.660885,
        -73.962124
    ],
    [
        40.660873,
        -73.962121
    ],
    [
        40.660861,
        -73.962118
    ],
    [
        40.660849,
        -73.962114
    ],
    [
        40.660837,
        -73.962111
    ],
    [
        40.660825,
        -73.962107
    ],
    [
        40.66054,
        -73.962026
    ],
    [
        40.66051,
        -73.962018
    ],
    [
        40.660481,
        -73.962011
    ],
    [
        40.660452,
        -73.962004
    ],
    [
        40.660423,
        -73.961997
    ],
    [
        40.660395,
        -73.961991
    ],
    [
        40.660367,
        -73.961984
    ],
    [
        40.66034,
        -73.961978
    ],
    [
        40.660314,
        -73.961972
    ],
    [
        40.660288,
        -73.961966
    ],
    [
        40.660262,
        -73.961961
    ],
    [
        40.660237,
        -73.961956
    ],
    [
        40.660212,
        -73.961951
    ],
    [
        40.660188,
        -73.961946
    ],
    [
        40.660164,
        -73.961941
    ],
    [
        40.660141,
        -73.961937
    ],
    [
        40.6569,
        -73.96141
    ],
    [
        40.656798,
        -73.961394
    ],
    [
        40.656696,
        -73.961383
    ],
    [
        40.656594,
        -73.961374
    ],
    [
        40.65649,
        -73.961368
    ],
    [
        40.656386,
        -73.961365
    ],
    [
        40.656281,
        -73.961365
    ],
    [
        40.656174,
        -73.961368
    ],
    [
        40.656067,
        -73.961373
    ],
    [
        40.655959,
        -73.961382
    ],
    [
        40.655851,
        -73.961393
    ],
    [
        40.655741,
        -73.961408
    ],
    [
        40.65563,
        -73.961425
    ],
    [
        40.655519,
        -73.961445
    ],
    [
        40.655406,
        -73.961468
    ],
    [
        40.655292,
        -73.961495
    ],
    [
        40.653888,
        -73.961938
    ],
    [
        40.653681,
        -73.962006
    ],
    [
        40.653472,
        -73.962074
    ],
    [
        40.65326,
        -73.962142
    ],
    [
        40.653046,
        -73.96221
    ],
    [
        40.652829,
        -73.962278
    ],
    [
        40.65261,
        -73.962347
    ],
    [
        40.652389,
        -73.962416
    ],
    [
        40.652165,
        -73.962486
    ],
    [
        40.651938,
        -73.962556
    ],
    [
        40.651709,
        -73.962626
    ],
    [
        40.651478,
        -73.962696
    ],
    [
        40.651244,
        -73.962767
    ],
    [
        40.651007,
        -73.962839
    ],
    [
        40.650768,
        -73.96291
    ],
    [
        40.650527,
        -73.962982
    ],
    [
        40.645866,
        -73.964421
    ],
    [
        40.645746,
        -73.964452
    ],
    [
        40.645626,
        -73.964479
    ],
    [
        40.645506,
        -73.964502
    ],
    [
        40.645386,
        -73.964522
    ],
    [
        40.645265,
        -73.964538
    ],
    [
        40.645144,
        -73.96455
    ],
    [
        40.645022,
        -73.964558
    ],
    [
        40.6449,
        -73.964563
    ],
    [
        40.644777,
        -73.964564
    ],
    [
        40.644654,
        -73.964561
    ],
    [
        40.644531,
        -73.964554
    ],
    [
        40.644407,
        -73.964544
    ],
    [
        40.644282,
        -73.96453
    ],
    [
        40.644157,
        -73.964512
    ],
    [
        40.644031,
        -73.964492
    ],
    [
        40.640927,
        -73.963891
    ],
    [
        40.635082,
        -73.962793
    ],
    [
        40.630739,
        -73.961973
    ],
    [
        40.630686,
        -73.961964
    ],
    [
        40.630634,
        -73.961956
    ],
    [
        40.630583,
        -73.961947
    ],
    [
        40.630533,
        -73.961938
    ],
    [
        40.630484,
        -73.961929
    ],
    [
        40.630437,
        -73.96192
    ],
    [
        40.63039,
        -73.961911
    ],
    [
        40.630344,
        -73.961901
    ],
    [
        40.6303,
        -73.961892
    ],
    [
        40.630256,
        -73.961883
    ],
    [
        40.630214,
        -73.961873
    ],
    [
        40.630173,
        -73.961863
    ],
    [
        40.630132,
        -73.961854
    ],
    [
        40.630093,
        -73.961844
    ],
    [
        40.630055,
        -73.961834
    ],
    [
        40.62927,
        -73.961639
    ],
    [
        40.629189,
        -73.961619
    ],
    [
        40.629109,
        -73.9616
    ],
    [
        40.629031,
        -73.961581
    ],
    [
        40.628955,
        -73.961563
    ],
    [
        40.62888,
        -73.961545
    ],
    [
        40.628807,
        -73.961529
    ],
    [
        40.628736,
        -73.961513
    ],
    [
        40.628667,
        -73.961497
    ],
    [
        40.628599,
        -73.961483
    ],
    [
        40.628533,
        -73.961469
    ],
    [
        40.628469,
        -73.961455
    ],
    [
        40.628407,
        -73.961443
    ],
    [
        40.628346,
        -73.961431
    ],
    [
        40.628287,
        -73.96142
    ],
    [
        40.628229,
        -73.961408
    ],
    [
        40.625039,
        -73.960803
    ],
    [
        40.617618,
        -73.959399
    ],
    [
        40.610171,
        -73.957986
    ],
    [
        40.610149,
        -73.957982
    ],
    [
        40.610127,
        -73.957978
    ],
    [
        40.610105,
        -73.957974
    ],
    [
        40.610084,
        -73.95797
    ],
    [
        40.610063,
        -73.957966
    ],
    [
        40.610043,
        -73.957963
    ],
    [
        40.610023,
        -73.95796
    ],
    [
        40.610003,
        -73.957956
    ],
    [
        40.609984,
        -73.957953
    ],
    [
        40.609965,
        -73.957951
    ],
    [
        40.609946,
        -73.957948
    ],
    [
        40.609928,
        -73.957946
    ],
    [
        40.60991,
        -73.957943
    ],
    [
        40.609892,
        -73.957941
    ],
    [
        40.609875,
        -73.957939
    ],
    [
        40.609748,
        -73.957922
    ],
    [
        40.609729,
        -73.95792
    ],
    [
        40.60971,
        -73.957918
    ],
    [
        40.609691,
        -73.957917
    ],
    [
        40.609673,
        -73.957915
    ],
    [
        40.609654,
        -73.957913
    ],
    [
        40.609636,
        -73.95791
    ],
    [
        40.609618,
        -73.957908
    ],
    [
        40.6096,
        -73.957906
    ],
    [
        40.609582,
        -73.957903
    ],
    [
        40.609564,
        -73.957901
    ],
    [
        40.609547,
        -73.957898
    ],
    [
        40.609529,
        -73.957896
    ],
    [
        40.609512,
        -73.957893
    ],
    [
        40.609495,
        -73.95789
    ],
    [
        40.609478,
        -73.957887
    ],
    [
        40.60867,
        -73.957734
    ],
    [
        40.608159,
        -73.95764
    ],
    [
        40.608138,
        -73.957636
    ],
    [
        40.608117,
        -73.957632
    ],
    [
        40.608096,
        -73.957628
    ],
    [
        40.608075,
        -73.957624
    ],
    [
        40.608055,
        -73.95762
    ],
    [
        40.608035,
        -73.957616
    ],
    [
        40.608016,
        -73.957611
    ],
    [
        40.607996,
        -73.957607
    ],
    [
        40.607977,
        -73.957602
    ],
    [
        40.607959,
        -73.957597
    ],
    [
        40.60794,
        -73.957592
    ],
    [
        40.607922,
        -73.957588
    ],
    [
        40.607904,
        -73.957582
    ],
    [
        40.607886,
        -73.957577
    ],
    [
        40.607869,
        -73.957572
    ],
    [
        40.607789,
        -73.957548
    ],
    [
        40.607776,
        -73.957545
    ],
    [
        40.607764,
        -73.957541
    ],
    [
        40.607751,
        -73.957538
    ],
    [
        40.607738,
        -73.957535
    ],
    [
        40.607725,
        -73.957532
    ],
    [
        40.607712,
        -73.957529
    ],
    [
        40.607699,
        -73.957526
    ],
    [
        40.607685,
        -73.957522
    ],
    [
        40.607672,
        -73.957519
    ],
    [
        40.607659,
        -73.957516
    ],
    [
        40.607645,
        -73.957513
    ],
    [
        40.607632,
        -73.957511
    ],
    [
        40.607618,
        -73.957508
    ],
    [
        40.607605,
        -73.957505
    ],
    [
        40.607591,
        -73.957502
    ],
    [
        40.5993,
        -73.955929
    ],
    [
        40.595246,
        -73.955161
    ],
    [
        40.590785,
        -73.954319
    ],
    [
        40.589295,
        -73.954106
    ],
    [
        40.589221,
        -73.954096
    ],
    [
        40.589147,
        -73.954087
    ],
    [
        40.589072,
        -73.954079
    ],
    [
        40.588997,
        -73.954072
    ],
    [
        40.588921,
        -73.954065
    ],
    [
        40.588844,
        -73.95406
    ],
    [
        40.588767,
        -73.954057
    ],
    [
        40.588689,
        -73.954054
    ],
    [
        40.58861,
        -73.954052
    ],
    [
        40.58853,
        -73.954051
    ],
    [
        40.58845,
        -73.954052
    ],
    [
        40.588369,
        -73.954054
    ],
    [
        40.588288,
        -73.954056
    ],
    [
        40.588205,
        -73.95406
    ],
    [
        40.588121,
        -73.954066
    ],
    [
        40.587758,
        -73.954096
    ],
    [
        40.587708,
        -73.954104
    ],
    [
        40.587657,
        -73.95411
    ],
    [
        40.587605,
        -73.954116
    ],
    [
        40.587552,
        -73.954121
    ],
    [
        40.587498,
        -73.954126
    ],
    [
        40.587443,
        -73.954131
    ],
    [
        40.587387,
        -73.954135
    ],
    [
        40.587329,
        -73.954139
    ],
    [
        40.587271,
        -73.954142
    ],
    [
        40.587211,
        -73.954145
    ],
    [
        40.58715,
        -73.954148
    ],
    [
        40.587088,
        -73.95415
    ],
    [
        40.587025,
        -73.954152
    ],
    [
        40.586961,
        -73.954154
    ],
    [
        40.586896,
        -73.954155
    ],
    [
        40.586783,
        -73.954153
    ],
    [
        40.58667,
        -73.954158
    ],
    [
        40.586556,
        -73.954167
    ],
    [
        40.586443,
        -73.954181
    ],
    [
        40.586329,
        -73.954201
    ],
    [
        40.586216,
        -73.954225
    ],
    [
        40.586103,
        -73.954254
    ],
    [
        40.585989,
        -73.954289
    ],
    [
        40.585876,
        -73.954328
    ],
    [
        40.585763,
        -73.954373
    ],
    [
        40.58565,
        -73.954422
    ],
    [
        40.585536,
        -73.954477
    ],
    [
        40.585423,
        -73.954536
    ],
    [
        40.58531,
        -73.954601
    ],
    [
        40.585198,
        -73.95467
    ],
    [
        40.578316,
        -73.959575
    ],
    [
        40.578268,
        -73.959607
    ],
    [
        40.578223,
        -73.959643
    ],
    [
        40.57818,
        -73.959681
    ],
    [
        40.578139,
        -73.959721
    ],
    [
        40.578101,
        -73.959765
    ],
    [
        40.578064,
        -73.959811
    ],
    [
        40.57803,
        -73.95986
    ],
    [
        40.577998,
        -73.959912
    ],
    [
        40.577969,
        -73.959967
    ],
    [
        40.577942,
        -73.960025
    ],
    [
        40.577917,
        -73.960085
    ],
    [
        40.577894,
        -73.960148
    ],
    [
        40.577873,
        -73.960214
    ],
    [
        40.577855,
        -73.960283
    ],
    [
        40.577839,
        -73.960354
    ],
    [
        40.577621,
        -73.961376
    ]
    ]

    query.run('b_train', JSON.stringify(bTrainCoords));

    insertMany(shapePoints);
}

async function processStaticGTFS() {
    /*
        1. Process data for list of stations
        2. Process data for list of train lines (routes)
        3. Process data for all the shape points
    */

    console.log("Reading static gtfs data");

    const stopsData = await getFile("stops.txt");
    const routesData = await getFile("routes.txt");
    const tripsData = await getFile("trips.txt");
    insertTrips(tripsData);

    //these files are humungous
    // const stopTimesData = await getFile("stop_times.txt");
    // insertStopTimes(stopTimesData); //push to database

    const shapesData = await getFile("shapes.txt");
    
    console.log("Processing static gtfs data");
    await processStationData(stopsData);


    //process train routes
    console.log("Processing routes data");
    await processRoutesData(routesData, tripsData);

    //process shape data
    console.log("Processing shape data");
    await processShapesData(shapesData);
}

async function fetchStaticGTFS() {
    const zipPath = await downloadGTFS();
    await extractGTFS(zipPath);
    await processStaticGTFS();
}

//do the same with stations & shapes
async function insertTrainRoutes(data) {
    const insertRoute = db.prepare(`
        INSERT OR REPLACE INTO train_routes (
            route_id, agency_id, short_name, long_name,
            description, route_type, route_url,
            route_color, route_text_color, sort_order,
            shapes_json
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    const insertOperation = async (obj) => {
        await insertRoute.run(
            obj.route_id,
            obj.agency_id,
            obj.route_short_name,
            obj.route_long_name,
            obj.route_desc,
            obj.route_type,
            obj.route_url,
            obj.route_color,
            obj.route_text_color,
            obj.route_sort_order,
            JSON.stringify(obj.shapes)
        );
    };

    for(const pt of data.keys()) {
        await insertOperation(data.get(pt));
    }
}

module.exports = {
    fetchStaticGTFS,
    getFile
};

/*
    Lacks - frequently updating data, requires server restart
*/